...
def main():
    ...
    parser.add_argument('-ob','--ouput_bam', action='store_true', help='ouput BAM files with reads falling into use/low_quality/unmap reads' )
    ...
    for (counter, read) in enumerate(samfile.fetch(until_eof = True)):
        ...
        SmaI_side_key=(str(samfile.getrname(read.tid)),SmaI_side)
    ...
    with open(file_name_root+'_SmaI_sites.txt', 'w') as output:
        for key, value in SmaI_DB.iteritems():
            ...
            if bed_like:
                chrom,pos = key
                output.write('{0}\t{1}\t{2}\t{3}\t{ID}\t{M}\t{U}'.format(chrom, pos-2, pos, percentage, **value)+"\n") # now prints CpG to access CCCGGG: pos-4, pos+2
            else:
